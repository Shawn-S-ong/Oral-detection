...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes):
        ...
        # resize_ratio = self.upsample_size[0]/imgs.shape[2]
        # target_resized = (resize_ratio * target)
        # target_resized = target_resized.astype('int')
        # bboxes_resized = (resize_ratio * bboxes)
...
if __name__ == '__main__':
    ...
    clean_data_path = 'C:/Users/sssnow/Desktop/data_implant_new/sagittal/'
    ...
    for count, path in enumerate(filenames):
        ...
        # plt.axis('off')
        #
        # rect = patches.Rectangle((label[1], label[0]), label[3], label[2], linewidth=2,
        ...
        sample, target, bboxes, coord32 = crop(data, label.squeeze(), bboxes)
        ...
        # plt.axis('off')
        #
        # rect = patches.Rectangle((target[1], target[0]), target[3], target[2], linewidth=2,
        #                          edgecolor='red', facecolor='none')
        # ax.add_patch(rect)
        # plt.show()

        data = [sample, label32, coord32]


        if count > test_th:
            np.save('test_data_2d/' + str(count+1) + '.npy', data)
        else:
            np.save('sagittal_implant_2d/' + str(count+1) + '.npy', data)
