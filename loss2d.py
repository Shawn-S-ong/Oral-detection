...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train=True):
        ...
        if len(pos_output) > 0:
            ...
            ph, pw, pdh, pdw = pos_output[:, 1], pos_output[:, 2], pos_output[:, 3], pos_output[:, 4]
            lh, lw, ldh, ldw = pos_labels[:, 1], pos_labels[:, 2], pos_labels[:, 3], pos_labels[:, 4]

            regress_losses = [
                self.regress_loss(ph, lh),
                self.regress_loss(pw, lw),
                self.regress_loss(pdh, ldh),
                self.regress_loss(pdw, ldw)]
            regress_losses_data = []
            for i in regress_losses:
                lossfig = i.data.item()
                regress_losses_data.append(lossfig)
            classify_loss = 0.5 * self.classify_loss(
                pos_prob.float(), pos_labels[:, 0].float()) + 0.5 * self.classify_loss(
                neg_prob.float(), neg_labels.float() + 1)
